            ...
            <div style={{ position: 'relative', paddingLeft: 4 }}>
              ...
              {state.upcoming.map((u, i) => (
                <div key={i} className="va-sans v1-timeline-row" style={{
                  ...
                  <div className="v1-timeline-when" style={{ fontSize: 11, color: '#9b4722', fontWeight: 600, letterSpacing: 0.5 }}>{u.when}</div>
                  <div className="v1-timeline-what" style={{ fontSize: 13, color: '#1d1a15', fontWeight: 500 }}>{u.what}</div>
...
function StatusDot({ status }) {
  ...
  return (
    <div title={s.label} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
      ...
      <span className="v1-status-label" style={{ fontSize: 11, color: '#57514a', fontWeight: 500, textTransform: 'uppercase', letterSpacing: 0.6 }}>
        {s.label}
      </span>
    </div>
  );
